...
for (let i = 0; i < 15; i++) {
    ...
    resultElement.innerHTML += `
        <div class="twoDice">
            <img src="${diceImages[dice1 - 1]}" alt="Dice showing ${dice1}" class="${dice1 > dice2 ? 'highlight' : ''}">
            <img src="${diceImages[dice2 - 1]}" alt="Dice showing ${dice2}" class="${dice2 > dice1 ? 'highlight' : ''}">
            <p>${result}</p>
        </div>
    `;
}
...
finalScoreElement.innerHTML = `
    <hr>
    <h2>${checkWinner()}</h2>
    <p>Endstand: <span class="spieler1 ${score1 > score2 ? 'highlight' : ''}">Spieler 1: ${score1}</span>, <span class="spieler2 ${score2 > score1 ? 'highlight' : ''}">Spieler 2: ${score2}</span></p>
`;
